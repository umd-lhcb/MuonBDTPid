...
void uBoostEval(){
  ...
  cout<<endl<<"Processing "<<ns<<" signal events from train/test sample:"<<endl;
  for(int e = 0; e < ns; e++){
  ...
  cout<<endl<<"Processing "<<ns<<" background events from train/test sample:"<<endl;
  for(int e3 = 0; e3 < nb; e3++){
    if(e3 % 10000 == 0) cout<<"...... "<<e3<<" events complete"<<endl;
    tb->GetEntry(e3);
    BDT = reader->EvaluateMVA("UBDT method");
    bkgdResponse = BDT;
    tbResponse->Fill();
  ...
  cout<<endl<<"Processing "<<nHighStat<<" signal events from independent high statisitics sample:"<<endl;
  for(int e4 = 0; e4 < nHighStat; e4++){
    if(e4 % 100000 == 0) cout<<"...... "<<e4<<" events complete"<<endl;
    t5e5->GetEntry(e4);
    BDT = reader->EvaluateMVA("UBDT method");
