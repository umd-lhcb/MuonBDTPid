...
int main() {
  ...
  TString IsMuonTightBranchName = "mu_isMuonTight";
  TString OutputBranchName = "BDTmu";
  ...
  // reader->AddVariable("TrackLikelihood",&TrackLikelihood);
  ...
  // reader->AddVariable("RichUsedAero",&RichUsedAero);this is run 2!
  ...
  // reader->AddVariable("CombDLLk",&);
  // reader->AddVariable("CombDLLp",&);
  // reader->AddVariable("CombDLLe",&);
  ...
  TFile *inputS = new TFile(filename, "update");
  TTree *ts = (TTree *)inputS->Get(treename);
  ...
  TTreeFormula mu_isMuonTight_tf("mu_isMuonTight", IsMuonTightBranchName, ts);
  ...
  Float_t signalResponse;
  TBranch *br = ts->Branch(OutputBranchName, &signalResponse);
